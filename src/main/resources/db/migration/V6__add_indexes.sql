...
CREATE INDEX idx_team_member_team_user_not_deleted ON team_member (team_id, user_id) WHERE deleted_at IS NULL;
CREATE INDEX idx_team_member_user_not_deleted ON team_member (user_id) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_match_post_location_gist ON match_post USING GIST (location) WHERE deleted_at IS NULL AND status = 'OPEN';

COMMENT ON INDEX idx_match_post_location_gist IS '지도 기반 매치 검색 (PostGIS GIST Index)';

-- 정렬: ORDER BY match_date ASC, id ASC (기본 정렬)
CREATE INDEX idx_match_post_status_date ON match_post (status, match_date ASC, id ASC) WHERE deleted_at IS NULL;

-- 정렬: ORDER BY created_at DESC, id DESC
CREATE INDEX idx_match_post_team_created ON match_post (team_id, created_at DESC, id DESC) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_team_created_id_not_deleted ON team (created_at DESC, id DESC) WHERE deleted_at IS NULL;
CREATE INDEX idx_team_leader_not_deleted ON team (leader_user_id) WHERE deleted_at IS NULL;
